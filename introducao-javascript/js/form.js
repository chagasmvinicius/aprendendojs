...
btnAdicionar.addEventListener('click', function (event) { //adiconando um listener para perceber quando tiver um "click" no meu botão "Adicionar"
    ...
    if (!validaPeso(peso) || !validaAltura(altura)) {
        ...
        regitrandoNaTabela(nome, peso, altura, gordura, calcularImcForm(peso, altura));
        dadosForm.reset();
...
function regitrandoNaTabela(pNome, pPeso, pAltura, pGordura, pImc) {
    ...
    setarValoresTds(novoRegistroTdNome, pNome, 'info-nome');
    let novoRegistroTdPeso = document.createElement('td');
    setarValoresTds(novoRegistroTdPeso, pPeso, 'info-peso');
    let novoRegistroTdAltura = document.createElement('td');
    setarValoresTds(novoRegistroTdAltura, pAltura, 'info-altura');
    let novoRegistroTdGordura = document.createElement('td');
    setarValoresTds(novoRegistroTdGordura, pGordura, 'info-gordura');
    let novoRegistroTdImc = document.createElement('td');
    setarValoresTds(novoRegistroTdImc, pImc, 'info-imc');
